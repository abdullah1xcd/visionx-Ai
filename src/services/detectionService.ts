...
class RealDetectionService {
  ...
  private uniqueCategoryTrackIds = {
    person: new Set<number>(), car: new Set<number>(), bicycle: new Set<number>(), camera: new Set<number>()
  };
  ...
  public async initModel(): Promise<boolean> {
    ...
    try {
      ...
      this.addLog(`FATAL: Failed to load detection model: ${err?.message || 'unknown error'}`);
      this.isModelLoading = false;
      return false;
    }
  }
  ...
  /** COCO-SSD classes supported by the current production model. */
  public mapCategory(cocoClass: string): CategoryType | null {
    ...
    // COCO-SSD does not provide a reliable generic surveillance-camera class.
    // Never reinterpret phones/laptops as cameras.
    return null;
  }
  ...
  public async detectFrame(imageSource: HTMLVideoElement | HTMLCanvasElement, frameNumber: number, confidenceThreshold = 0.4): Promise<DetectionResult[]> {
    ...
    try {
      ...
      this.lastInferenceTimeMs = Math.round(performance.now() - t0);
      const now = performance.now();
      this.inferenceFrameTimes.push(now);
      if (this.inferenceFrameTimes.length > 30) this.inferenceFrameTimes.shift();
      ...
      for (const pred of rawPredictions) {
        ...
        const category = this.mapCategory(pred.class);
  ...
  private updateSpatialTracker(detections: { category: CategoryType; class_name: string; confidence: number; bbox: [number, number, number, number] }[], frameNumber: number): DetectionResult[] {
    ...
    for (const det of detections) {
      ...
      for (let i = 0; i < this.activeTracks.length; i++) {
        ...
        const iou = this.calculateIoU(det.bbox, this.activeTracks[i].bbox);
        if (iou > bestIou) { bestIou = iou; bestTrackIdx = i; }
      }
      let trackingId: number;
      if (bestTrackIdx >= 0) {
        matched.add(bestTrackIdx);
        const track = this.activeTracks[bestTrackIdx];
        track.bbox = det.bbox; track.confidence = det.confidence; track.lastSeenFrame = frameNumber; track.hits += 1; trackingId = track.id;
      } else {
        trackingId = this.nextTrackId++;
        this.activeTracks.push({ id: trackingId, category: det.category, class_name: det.class_name, bbox: det.bbox, confidence: det.confidence, lastSeenFrame: frameNumber, hits: 1 });
  ...
  private calculateIoU(b1: [number, number, number, number], b2: [number, number, number, number]) {
    ...
    const union = a1 + a2 - intersection;
  ...
  public async testAiEngine(imageSource: HTMLVideoElement | HTMLCanvasElement): Promise<EngineTestResult> {
    ...
    try {
      ...
      return { modelLoaded: true, frameCaptured: true, inferenceSuccess: false, device: this.deviceName, inferenceTimeMs: 0, detectedCount: 0, detections: [], timestamp };
    }
  }

  public getDiagnostics(inputWidth = 0, inputHeight = 0, cameraFps = 0): DiagnosticsData {
    const counts = { person: 0, car: 0, bicycle: 0, camera: 0 };
    for (const t of this.activeTracks) counts[t.category]++;
    ...
    return { modelName: 'COCO-SSD MobileNetV2', modelStatus: this.engineStatus, device: this.deviceName, inputResolution: `${inputWidth}x${inputHeight}`, inferenceResolution: `${inputWidth}x${inputHeight}`, inferenceFps, cameraFps, lastInferenceMs: this.lastInferenceTimeMs, totalDetectionsCurrentFrame: this.activeTracks.length, currentCounts: counts, uniqueTrackedCounts: { person: this.uniqueCategoryTrackIds.person.size, car: this.uniqueCategoryTrackIds.car.size, bicycle: this.uniqueCategoryTrackIds.bicycle.size, camera: this.uniqueCategoryTrackIds.camera.size }, trackingMethod: 'IoU spatial tracking', recentLogs: [...this.diagnosticLogs] };
  }

  public resetTracking() {
    this.nextTrackId = 1; this.activeTracks = []; this.uniqueSeenTrackIds.clear();
    Object.values(this.uniqueCategoryTrackIds).forEach(s => s.clear());
  }

  public getQualityStatus(): DetectionQualityStatus { return this.engineStatus === 'ACTIVE' ? 'GOOD' as DetectionQualityStatus : 'POOR' as DetectionQualityStatus; }
